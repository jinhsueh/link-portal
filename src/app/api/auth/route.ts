...
export async function POST(req: NextRequest) {
  ...
  const hash = await bcrypt.hash(password, 12)
  const user = await createUserWithDefaults({ username, email, name, passwordHash: hash })
...
function setSessionAndRespond(user: { id: string; username: string; name: string | null }) {
  ...
  res.cookies.set(SESSION_COOKIE, signSession(user.username), SESSION_COOKIE_OPTIONS)
  return res
}
