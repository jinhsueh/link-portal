...
export default function AnalyticsPage() {
  ...
  const METRIC_CARDS = [
    { icon: MousePointer, label: a.metricClicks, value: totalClicks, color: 'var(--color-primary)' },
    { icon: Eye, label: a.metricViews, value: totalViews, color: '#38A169' },
    { icon: Users, label: a.metricBlocks, value: totalBlocks, color: '#D69E2E' },
    { icon: TrendingUp, label: a.metricCtr, value: ctr, color: '#805AD5' },
  ]
  ...
  return (
    <AdminShell username={username} role={role} effectivePlan={effectivePlan} trialDaysLeft={trialDaysLeft}>
      <div className="max-w-4xl mx-auto px-4 py-8">
        <h1 className="font-bold text-lg mb-6" style={{ color: 'var(--color-text-primary)' }}>{a.pageTitle}</h1>
        ...
        <div className="mb-8" style={{ background: 'white', border: '1px solid var(--color-border)', borderRadius: 16, boxShadow: 'var(--shadow-sm)' }}>
          <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--color-border)' }}>
            <h2 className="font-bold" style={{ color: 'var(--color-text-primary)' }}>{a.dailyTitle}</h2>
            <p className="text-sm" style={{ color: 'var(--color-text-muted)' }}>{a.dailyHint}</p>
          </div>
        ...
        {referrers.length > 0 && (
          <div className="mb-8" style={{ background: 'white', border: '1px solid var(--color-border)', borderRadius: 16, boxShadow: 'var(--shadow-sm)' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--color-border)' }}>
              <h2 className="font-bold" style={{ color: 'var(--color-text-primary)' }}>{a.referrerTitle}</h2>
              <p className="text-sm" style={{ color: 'var(--color-text-muted)' }}>{a.referrerHint}</p>
            </div>
            <div>
              {referrers.map((r, i) => {
                ...
                return (
                  <div key={r.source} style={{ padding: '14px 24px', borderBottom: i < referrers.length - 1 ? '1px solid var(--color-border)' : 'none' }}>
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className="text-xs font-bold" style={{ color: 'var(--color-text-muted)' }}>{r.count} {a.clicksSuffix}</span>
                    </div>
        ...
        <div style={{ background: 'white', border: '1px solid var(--color-border)', borderRadius: 16, boxShadow: 'var(--shadow-sm)' }}>
          <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--color-border)' }}>
            <h2 className="font-bold" style={{ color: 'var(--color-text-primary)' }}>{a.blockRankTitle}</h2>
            <p className="text-sm" style={{ color: 'var(--color-text-muted)' }}>{a.blockRankHint}</p>
          </div>
          {blocks.length === 0 ? (
                  ...
                  <div key={block.id} style={{ padding: '16px 24px', borderBottom: i < blocks.length - 1 ? '1px solid var(--color-border)' : 'none' }}>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-4 text-xs" style={{ color: 'var(--color-text-muted)' }}>
                        <span><span className="font-bold" style={{ color: 'var(--color-text-primary)' }}>{block.clicks}</span> {a.clicksSuffix}</span>
                        <span><span className="font-bold" style={{ color: 'var(--color-text-primary)' }}>{block.views}</span> {a.viewsLabel}</span>
                      </div>
                    </div>
                    <div style={{ height: 4, background: 'var(--color-surface)', borderRadius: 9999 }}>
